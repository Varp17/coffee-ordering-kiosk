...
def inspect_range(elem, current_transform_y=0):
    ...
    if tag in ['rect', 'path', 'text', 'image', 'g']:
        # If it is a group with y-transform, or element with y
        if 4100 <= actual_y <= 5200:
    ...
    for child in elem:
        inspect_range(child, total_y)

print("Inspecting SVG elements in y-range 4100 to 5200:")
inspect_range(root)
